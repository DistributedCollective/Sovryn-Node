...
export async function queryAllForTokenPair(tokenA, tokenB) {
    ...
    const priceOracle = getPriceOracle(priceOracleAddress);
...
export async function querySovLiquidityPoolData(sovConverterAddress) {
    const sovPool = getSovLiquidityPool(sovConverterAddress);
...
export function getSovLiquidityPool(converterAddress) {
...
export function getPriceOracle(priceOracleAddress) {
    return new web3.eth.Contract(
